...
| `scripts/run_qemu_socket.sh` | 启动 QEMU（socket脚本） | `bash scripts/run_qemu_socket.sh [BUILDROOT_DIR] [QEMU_RUN_SCRIPT]` |
...
#### 改造方法
将参考脚本中的 QEMU 启动参数做如下替换：

1. **移除** `-nographic` 参数
2. **添加** socket 串口参数：
```bash
-chardev socket,id=serial0,path=/tmp/qemu_cxl.sock,server=on,wait=off \
-serial chardev:serial0 \
-display none
```
3. 将改造后的命令在后台启动 QEMU（`&` 或 `is_background=true`）
4. 通过 socat 连接 socket 执行命令：
```bash
# 发送单条命令
echo "<command>" | socat - UNIX-CONNECT:/tmp/qemu_cxl.sock

# 交互式执行（推荐，可读取输出）
socat UNIX-CONNECT:/tmp/qemu_cxl.sock -
```

> **注意**：每次启动 QEMU 前需确认 `/tmp/qemu_cxl.sock` 不存在（旧 QEMU 进程未关闭），如有则先 `kill` 旧进程
