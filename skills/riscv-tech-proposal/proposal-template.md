...
> 此模板基于 4 份已有的 RISC-V 国际社区提案提取，涵盖 ISA 扩展、ACPI 表、IOMMU 特性、PMU 扩展等类型。
...
以下文件位于 `/rvhome/chenp/.qoder/proposal/`，可作为写作参考：

| 文件 | 类型 | 特点 |
|------|------|------|
| ACPI Support for RISC-V RERI Proposal of Work.docx | ACPI 表/RAS | 背景+跨架构对比模式清晰 |
| Proposal for Hardware-Accelerated CQs in RISC-V IOMMU.docx | IOMMU 扩展 | 寄存器设计详尽，有架构图 |
| Proposal of CPU Utilization Counter(Sscucnt) Extension.docx | ISA 扩展 | CSR 定义完整，公式推导严谨 |
| RISC-V Uncore Performance Monitoring Unit (PMU) Proposal of Work.docx | PMU 扩展 | 目标/里程碑最完整，设计原则明确 |
